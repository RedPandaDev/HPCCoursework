...
int main (int argc, const char * argv[]) {
	...
	int doBlur (int *h_Rnew, int *h_R,int *h_Gnew, int *h_G,int *h_Bnew, int *h_B,int colsize, int rowsize) {
	nblurs = 10;
	printf("%i\n",nblurs);
 ...
 //    memset(h_R, 0, sizeof h_R);
    ...
    for(k=0;k<nblurs;k++){
		performUpdatesKernel<<<dimGrid,dimBlock>>>(h_Rnew,h_R,h_Gnew,h_G,h_Bnew,h_B,colsize,rowsize);
        doCopyKernel<<<dimGrid,dimBlock>>>(h_Rnew,h_R,h_Gnew,h_G,h_Bnew,h_B,colsize,rowsize);
